...
public class Runner {
	...
	public void testCase1() {
		...
		if (ConfigFileUtil.Environment().equalsIgnoreCase("dev")) {
			...
			SelectBrowser.BrowserandURL(ConfigFileUtil.AsahiURLAddressDEV(), ConfigFileUtil.Browser());

		} else if (ConfigFileUtil.Environment().equalsIgnoreCase("sit")) {
			SelectBrowser.BrowserandURL(ConfigFileUtil.AsahiURLAddressSIT(), ConfigFileUtil.Browser());
		}

		htmlReporter = new ExtentHtmlReporter(
				System.getProperty("user.dir") + "/test-output/STMExtentReportforAsahi.html");
